...
% obtain pressure info CHECK FILE NAME LAY-OUT AND correct sscanf.m IF NECESSARY
data(1,kk) = sscanf(current_file, '%3dN.txt'); % 'rot%3dN.txt' for rot data, '%3dN for elong data % obtain pressure in kPa from string name
...
switch answer
    case 'Yes'
       save (['ForceData_',datestr(datetime,30),'.mat'], 'data') ;
    case 'No'
       return         
end
