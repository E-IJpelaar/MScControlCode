function [E]= errorFunction(x,theta_d,z_ds,y_ds,z_mid_avgs,y_mid_avgs,Nmode,shape)

q = x;                                             % set q equal to optimized variable x
[theta_fkin,x_fkin,z_fkin] = funcKinematics(Nmode,shape,q);   % based on q determine [x-z] based on forward kinematics
...
E = sum((z_mid_avgs - x_q).^2 + (y_mid_avgs-z_q).^2 + (arclen_mean-arclen).^2  + (z_ds-x_fkin(end)).^2 + (y_ds-z_fkin(end)).^2 + (theta_d-theta_fkin).^2 );
