...
for ii = 1:length(NFix)-1
    Mmesh.Segment{ii,1} = ((NFix(ii)+1):(NFix(ii+1)-1)).';
end
...
%---------------------------------------------------------------------- set
function Mmesh = inductance(Mmesh)
...
Mmesh.Inductance = mu0*sum(sum(Ltmp))/(4*pi);
