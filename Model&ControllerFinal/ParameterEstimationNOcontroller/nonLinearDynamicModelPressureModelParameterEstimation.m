function dxdt = nonLinearDynamicModelPressureModelParameterEstimation(t,x,D,H,L0,m,w,d,Nmode,shape,space_step,dt)
k = x(1);                  
e = x(2);
p1  = x(5);
p2  = x(6);

p = [p1;p2];
q = [k;e];

%% Pressure model dp/dt = -0.28p(t) + 2.68V(t)
% C_p1 = diag([0.285336645573371 , 0.285336645573371]);
% C_p2 = diag([2.687852328855003,2.687852328855003]);

% Updated pressure model for sinus input
% C_p1 = diag([0.571455683571649 , 0.571455683571649]);
% C_p2 = diag([3.935359161537569 , 3.935359161537569]);

% Updated pressure model for step 9V
C_p1 = diag([0.408176851169921 , 0.408176851169921]);
C_p2 = diag([2.851102840226544 , 2.851102840226544]);





%% Determine Mass and Jacobian
[M,J]  = massMatrix(k,e,L0,m,w,d,Nmode,shape,space_step);
Jc = J(4:2:6,:);
invM = inv(M);


%% Stiffness matrix
% Calculate stiffness at instant x
Kk = bendStiffness(k);
Ke = elongStiffness(e);
% K = diag([30*Kk,4*Ke]);       % Stiffness matrix with mapped H
K = diag([2*Kk,4*Ke]);        % Stiffness matrix with old H

% Non-linear state-space
O2 = zeros(2,2);
I2 = eye(2,2);

%% 

if t < 2
    U(1) = 0;
    U(2) = 0;
elseif   t >= 2 && t < 22
   U(1) = 9;
   U(2) = 0;
else
   U(1) = 0;
   U(2)= 0;
end

U = U'; 

%% State-space update
% dynamics matrix
A = [ O2     , I2     , O2  ;
     -invM*K ,-invM*D , O2  ;
     O2     ,  O2    , -C_p1];

% input matrix
B = [  O2  ,  O2 ;
      invM*H ,  O2 ;
       O2  ,  C_p2 ];  % jacobian control

% input vector
u = [ p;
      U];
 
dxdt = A*x + B*u;

end



