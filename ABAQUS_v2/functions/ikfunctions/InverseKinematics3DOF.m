function  [theta_opt, xy,q_opt]  = InverseKinematics3DOF(x_d,epsilon,Nmode,L0)
%% Initial conditions
...
%% Check initial guess
[r,R,~,~] = Kinematics(q0,Nmode,shape,L0);      % Check for intial guess
...
while norm(e) > epsilon        % loop until error is smaller than max error norm & maxium iterations is not exceeded
        ...
        [r,R,l,Ba] = Kinematics(q0,Nmode,shape,L0); % call forward kinematics script
        ...
        if abs(e_norms(1)-e_norms(2)) <= 1e-8
            warning('Optimization has stopped, error does not further converge. Increasing N_mode might help. Or consider using more shape functions in approximation')
            break;
        end
...
[r_opt,R_opt,~,~] = Kinematics(q_opt,Nmode,shape,L0);  % Check output of algorith
