...
%         
        ...
        figure(2)
        plot(t0,dp,'LineWidth',1.5)       
        hold on;grid on;
        xlabel('Time [s]','FontSize',12);ylabel('Pressure [kPa]','FontSize',12)
        %title('Step w/ airtank & actuator')

end

figure(2)
legend('1V','2V','3V','3V','5V','6V','7V','8V','9V','Orientation','horizontal','NumColumns',6,'Location','south','FontSize',12)


figure(3)
plot(Vsteps,tau_approx,'bx','MarkerSize',14,'LineWidth',2)
hold on;grid on;
yline(mean(tau_approx),'r--')
xlabel('Volt [V]');ylabel('Time constant \tau [1/s]')

figure(4)
plot(Vsteps,K_approx,'bx','MarkerSize',14,'LineWidth',2)
hold on;grid on;
yline(mean(K_approx),'r--')
...
Vtau = [3:9]; % Volts used to estimate parameter tau
